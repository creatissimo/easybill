...
class Position implements ArrayAccess
{
    ...
    public function listInvalidProperties()
    {
        ...
        $allowed_values = $this->getPriceTypeAllowableValues();
        if (!in_array($this->container['price_type'], $allowed_values)) {
            $invalid_properties[] = sprintf(
    ...
    {
        $allowed_values = $this->getPriceTypeAllowableValues();
        if (!is_null($price_type) && !in_array($price_type, $allowed_values)) {
            throw new \InvalidArgumentException(
                sprintf(
                    "Invalid value for 'price_type', must be one of '%s'",
                    implode("', '", $allowed_values)
